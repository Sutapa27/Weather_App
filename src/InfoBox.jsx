...
export default function InfoBox({ info, error }) {
  ...
  const HAZE_URL =
    "https://images.unsplash.com/photo-1489179070830-36477bd83c93?q=80&w=1480&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D";
  const RAIN_URL =
  ...
  const getBackground = () => {
    if (info.description.includes("rain")) return RAIN_URL;
    if (info.description.includes("snow")) return SNOW_URL;
    if (info.description.includes("cloud")) return CLOUD_URL;
    if (info.description.includes("clear")) return CLEAR_URL;
    if (info.description.includes("thunder")) return THUNDER_URL;
    if (info.description.includes("mist") || info.description.includes("fog"))
      return HAZE_URL;
    return CLEAR_URL;
  };
  ...
  return (
    <Card sx={{ maxWidth: 400, minWidth: 300 }}>
      <CardMedia component="img" height="200" image={getBackground()} alt="weather" />
      <CardContent>
        <Typography gutterBottom variant="h5" component="div">
          ...
          ) : info.description.includes("mist") || info.description.includes("fog") ? (
            <CloudIcon color="disabled" />
          ) : (
            <WbSunnyIcon sx={{ color: yellow[500] }} />
          )}
        ...
        <Typography variant="body2" sx={{ color: "text.secondary" }} component={"span"}>
          ...
          <p>
            The weather can be described as <i>{info.description}</i> and it feels like {info.feelsLike}&deg;C
          </p>
        </Typography>
      </CardContent>
    </Card>
